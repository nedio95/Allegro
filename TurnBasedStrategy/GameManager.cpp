...
bool GameManager::InitialiseAllegro()
{
	...
	if (!al_install_mouse())
	{
		std::cout << "Mouse initialisation failed" << std::endl;
		std::cout << "Press any key to exit" << std::endl;
		_getch();
		return 0;
	}
	...
	// register mouse with the event queue, so keypress events can be processed
	...
	while (al_get_next_event(event_queue, &allegro_event))
	{
		...
		if (allegro_event.type == ALLEGRO_EVENT_KEY_DOWN)
		{
			...
			case ALLEGRO_KEY_D:
				...
				break;
			case ALLEGRO_KEY_Q:
				m_inputManager->OnKeyDown(m_inputManager->KEY_Q);
				break;
			}
		...
		else if (allegro_event.type == ALLEGRO_EVENT_KEY_UP)
		{
			...
			case ALLEGRO_KEY_D:
				...
				break;
			case ALLEGRO_KEY_Q:
				m_inputManager->OnKeyUp(m_inputManager->KEY_Q);
				break;
			}
		}
		...
		{
			if (allegro_event.mouse.button == 1)
			{
				float m_mouse_x = allegro_event.mouse.x;
				float m_mouse_y = allegro_event.mouse.y;
			
				//std::cout << "x =" << m_mouse_x << " y = " << m_mouse_y << std::endl;
			}
		}
...
void GameManager::LogicUpdate()
{
	...
	*/
	...
	case Scene::NEW_GAME:
		...
		std::cout << "Press [Esc] at any time to perform special attack RAGEQUIT " << std::endl;
		CurrentScene = new StrategyLevel("1");
		return;
	case Scene::LOAD:
		return;
	case Scene::QUIT:
...
{
	delete CurrentScene;
	CurrentScene = new StrategyLevel(levelNumber);
}